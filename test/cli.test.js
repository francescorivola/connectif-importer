...
t.test('cli should create import uploading csv file then check progress until import is finished without errors', async t => {
...
t.test('cli should create import uploading csv file then check progress until import is finished with errors', async t => {
  const postScope = nock('https://api.connectif.cloud', {
    reqheaders: {
      authorization: 'apiKey myApiKey',
      'content-type': 'multipart/form-data; boundary=--------------------------515890814546601021194782'
  ...
  const getScope = nock('https://api.connectif.cloud', {
    ...
    .reply(200, { status: 'inProgress', success: 500, errors: 0 })
    .get('/imports/6072374b4d3ceb76e44d4323')
    .reply(200, { status: 'finished', success: 900, errors: 100, errorReportFileUrl: 'https://site.com/error-report.csv' })
...
t.test('cli should create import uploading csv file then throw an error if import finished in error state', async t => {
  const postScope = nock('https://api.connectif.cloud', {
    reqheaders: {
      authorization: 'apiKey myApiKey',
      'content-type': 'multipart/form-data; boundary=--------------------------515890814546601021194782'
...
t.test('cli should throw error if got an api response error while creating import', async t => {
  const postScope = nock('https://api.connectif.cloud', {
    reqheaders: {
      authorization: 'apiKey myApiKey',
      'content-type': 'multipart/form-data; boundary=--------------------------515890814546601021194782'
...
t.test('cli should throw error if got an api response error while checking import progress', async t => {
  const postScope = nock('https://api.connectif.cloud', {
    reqheaders: {
      authorization: 'apiKey myApiKey',
      'content-type': 'multipart/form-data; boundary=--------------------------515890814546601021194782'
